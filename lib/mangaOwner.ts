 ...
 * an admin. Returns `{ err }` (a ready-to-return error response) or
 * `{ manga, userId }`. Use `if ("err" in r) return r.err;` then read `r.manga`.
 */
export async function resolveOwnedManga(
  rawSlug: string
): Promise<{ err: NextResponse } | { manga: { id: string; translatorId: string | null }; userId: string }> {
...
export async function resolveOwnedChapter(
  chapterId: string
): Promise<{ err: NextResponse } | { chapter: { id: string; mangaId: string; mangaSlug: string }; userId: string }> {
  const session = await auth();
  if (!session?.user) return { err: apiError("AUTH-007", 401) };
  const role = (session.user as { role?: string }).role;
  if (role !== "TRANSLATOR" && role !== "ADMIN") return { err: apiError("AUTH-008", 403) };
  ...
  return { chapter: { id: chapter.id, mangaId: chapter.mangaId, mangaSlug: chapter.manga.slug }, userId };
}
